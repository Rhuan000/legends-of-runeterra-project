...
pub async fn puuid_request(user: User) -> Result<User, Box<dyn Error>> {
    //starting variable
    dotenv::dotenv().ok();
    let token = env::var("RIOT_API_TOKEN").expect("RIOT_API_TOKEN not set");

    let url = format!("https://americas.api.riotgames.com/riot/account/v1/accounts/by-riot-id/{}/{}", user.game_name, user.tag_line);
    println!("{}", token);
